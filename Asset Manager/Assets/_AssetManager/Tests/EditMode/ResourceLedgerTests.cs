...
namespace AssetManager.Tests
{
    public sealed class ResourceLedgerTests
    {
        ...
        [Test]
        public void InvestmentPhilosophyGainCapsEachTypeWithoutCombinedTotalCap()
        {
            var run = RunBootstrapper.CreateNewRun(RunStaticDataSet.CreateMvpDefaults());
            ...
            var result = ResourceLedger.AddInvestmentPhilosophy(run, ResourceType.Patience, 5);

            Assert.That(result.Run.Resources.Reading, Is.EqualTo(5));
            Assert.That(result.Run.Resources.Meditation, Is.EqualTo(5));
            Assert.That(result.Run.Resources.Patience, Is.EqualTo(5));
            Assert.That(result.Run.Resources.InvestmentPhilosophyTotal, Is.EqualTo(15));
            Assert.That(result.GainedAmount, Is.EqualTo(5));
            Assert.That(result.DiscardedAmount, Is.EqualTo(0));
        ...
        [Test]
        public void CashAndDealAreExcludedFromInvestmentPhilosophyCapAndDealHasNoHoldingCap()
        {
            var run = RunBootstrapper.CreateNewRun(RunStaticDataSet.CreateMvpDefaults());
            run = ResourceLedger.AddInvestmentPhilosophy(run, ResourceType.Reading, 5).Run;
            run = ResourceLedger.AddInvestmentPhilosophy(run, ResourceType.Meditation, 5).Run;
            ...
            Assert.That(result.GainedAmount, Is.EqualTo(4));
            Assert.That(result.DiscardedAmount, Is.EqualTo(0));
            Assert.That(result.Message, Is.Empty);
        }

        [Test]
        public void InvestmentPhilosophyMasteryGainCapsEachTypeAtFive()
        {
            var run = RunBootstrapper.CreateNewRun(RunStaticDataSet.CreateMvpDefaults());
            run = ResourceLedger.AddInvestmentPhilosophyMastery(run, ResourceType.Reading, 4).Run;

            var result = ResourceLedger.AddInvestmentPhilosophyMastery(run, ResourceType.Reading, 3);

            Assert.That(result.Run.InvestmentPhilosophyMastery.Reading, Is.EqualTo(5));
            Assert.That(result.Run.InvestmentPhilosophyMastery.Meditation, Is.EqualTo(0));
            Assert.That(result.Run.InvestmentPhilosophyMastery.Patience, Is.EqualTo(0));
            Assert.That(result.GainedAmount, Is.EqualTo(1));
            Assert.That(result.DiscardedAmount, Is.EqualTo(2));
            Assert.That(result.Message, Is.EqualTo("투자 철학 마스터리 한도: 독서 +2 버림"));
        }
