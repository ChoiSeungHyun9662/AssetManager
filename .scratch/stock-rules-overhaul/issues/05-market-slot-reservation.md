...
- [ ] 예약은 별도 예약 구역이 아니라 시장 슬롯 상태로 관리된다.
- [ ] 주식 카드만 예약할 수 있고 소모형 자원 카드는 예약할 수 없다.
- [ ] 동시에 최대 3개의 주식을 예약할 수 있다.
- [ ] 예약된 주식은 시장 테이프 진행, 갱신, 당김 중에도 같은 슬롯에 남는다.
- [ ] 예약 액션은 딜 +1을 시도하고, 월세 밀림 +1을 적용하고, 파산 여부를 확인한다.
- [ ] 딜이 이미 최대치여도 예약은 성공하고 초과 딜만 버려진다.
- [ ] 예약 액션은 영업일을 소비하지만 즉시 시장 테이프를 진행하지 않는다.
- [ ] 예약된 주식을 구매하면 예약 상태가 해제되고, 시장 슬롯 빈칸은 시장 테이프 당김으로 메워진다.
