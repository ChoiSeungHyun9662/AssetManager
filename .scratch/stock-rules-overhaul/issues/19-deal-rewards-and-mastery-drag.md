...
플레이어는 리소스 HUD의 딜 스택에서 딜을 드래그해 독서, 명상, 인내 HUD lane에 놓을 수 있다. 성공하면 딜 1개를 소모하고 해당 투자 철학 마스터리를 1 올린다. 딜이 0개면 드래그할 수 없다.

딜 칩을 호버하거나 드래그하면 `드래그하여 투자 철학에 놓으면, 이 칩을 소모하여 해당 투자 철학 마스터리를 1 획득합니다.` 안내 패널을 보여준다. 드래그 중에는 원래 딜 칩 하나를 잠시 숨기고, 딜 칩 이미지와 안내 패널을 포인터에 붙여 움직인다. 안내 패널의 우측 하단 모서리는 포인터 위치에 맞춰진다. 철학 lane 밖에 놓으면 취소되고, 숨겼던 딜 칩을 다시 보여준다. 이미 마스터리 5인 철학에 놓으면 실패 메시지를 보여주되 딜은 소모하지 않는다.
