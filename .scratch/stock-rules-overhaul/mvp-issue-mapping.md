...
6. 시장 슬롯 예약
   - 예약 구역을 제거한다.
   - 별도 Reservation Panel과 예약 카드 버튼 목록을 제거한다.
...
8. 주식 매도
   ...
   - 보유 주식 Card Button 호버 시 Sell Button을 표시한다.
   - Card Button과 Sell Button 사이 커서 이동 중에는 Sell Button을 유지하고, 둘 다 벗어나면 숨긴다.
...
12. 주식 개편 전용 QA
    ...
    - 1x8 시장, 당김, 예약, 소모형 자원 카드, 압축된 보유 주식 카드 행, 호일, 호버 기반 매도 버튼, 월세 밀림, 파산, 최종 정산을 검증한다.
