...
시장 카드 상세보기에서 전문 자원 비용 슬롯과 딜을 사용해 결제 상태를 만들고, 매수 버튼으로 시장 카드를 보유 자산으로 전환한다. 매수 확정 전에는 자원을 소비하지 않고, 검증 실패 시 자원과 카드와 시장 상태를 바꾸지 않아야 한다.

## Code work

- PurchasePayment module을 구현한다.
- 카드의 전문 자원 비용을 PaymentSlotState 목록으로 만든다.
- 전문 자원 칩 클릭 배치와 회수, 딜 클릭 배치와 회수를 지원한다.
- 딜은 모든 전문 자원 비용 슬롯에 배치 가능하게 한다.
...
- 보유 자원 영역의 리서치, 신용, 원자재, 딜을 클릭해 슬롯에 배치할 수 있게 연결한다.
