...
시장 카드 상세보기에서 예약 버튼을 눌러 카드를 예약 슬롯으로 이동시키고, 딜 +1, 환매 압력 +1, 예약한 자리 보충, 시장 테이프 진행, 영업일 종료를 처리한다. 예약은 확인 상태 없이 즉시 실행되며, 예약 슬롯이 가득 찼을 때는 실행되지 않고 영업일도 소비하지 않아야 한다.
...
- 예약 버튼 활성 조건은 예약 슬롯 여유 있음으로 둔다.
- 딜이 3/3이어도 예약은 가능하게 한다.
- 예약 실행 시 비용 슬롯에 올린 칩이 있으면 자동 회수하고 결제 상태를 초기화한다.
- 선택한 시장 카드를 Reserved 상태로 바꾸고 첫 빈 예약 슬롯으로 이동한다.
- 예약으로 빈 시장 슬롯을 먼저 보충한 뒤 시장 테이프를 진행한다.
...
- 예약 슬롯이 가득 차면 예약 버튼을 비활성화하고 예약 슬롯 가득 참 메시지를 표시한다.
- 딜 초과 폐기 메시지와 환매 압력 증가 표시를 메시지 영역 또는 HUD에 연결한다.
- 예약 후 예약 슬롯에 카드 요약 UI가 표시되게 연결한다.
- 예약으로 인한 시장 테이프 보충과 진행이 화면에 반영되게 연결한다.

## Verification

- Play Mode에서 시장 카드 상세보기의 예약 버튼을 누르면 예약 슬롯에 카드가 들어간다.
- 예약 시 딜이 1 증가하고 환매 압력이 1 증가한다.
- 딜 3/3 상태에서도 예약은 실행되고 추가 딜 폐기 메시지가 표시된다.
- 예약 슬롯 3/3 상태에서는 예약 버튼이 비활성화되고 클릭해도 영업일이 소비되지 않는다.
- 예약 후 시장 테이프는 예약한 자리 보충 후 진행된다.
...
- [ ] 예약 슬롯은 최대 3개다.
- [ ] 예약 슬롯이 가득 차면 예약은 실행되지 않고 영업일도 소비하지 않는다.
- [ ] 딜 3/3이어도 예약 가능하며 추가 딜만 폐기된다.
- [ ] 예약은 환매 압력 +1을 발생시키고 즉시 한도 검사를 한다.
- [ ] 예약한 자리는 먼저 보충한 뒤 시장 테이프를 진행한다.
