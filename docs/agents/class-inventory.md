...
| `AssetCardData` | `Runtime/RunModels.cs` | Static market card definition: stock costs, 희귀도, base value/dividend, authored foil value/dividend, min/max deck copy counts, and tags, or consumable resource provided-resource data. The type name remains from the MVP asset-card implementation. |
...
| Type | File | Purpose |
| --- | --- | --- |
| `MainGameShellBootstrap` | `Runtime/MainGameShellBootstrap.cs` | Runtime orchestrator: owns `CurrentRun`, wires buttons plus zone-aware market/reserved card clicks to rule services, refreshes visible UI, and leaves the old GainLiquidity entry disconnected from the new play flow. |
