...
매수 출처에 따라 처리 규칙이 다르다.

| 매수 출처 | 매수 가능 | 예약 가능 | 매수 후 시장 처리 |
|---|---:|---:|---|
| MarketTape | O | O | 산 자리 보충 |
| Reserved | O | X | 예약 구역 비움 |
...
슬롯에 해당 전문 자원 칩 또는 딜이 배치되면 결제 완료 상태로 본다.
...
예약 구역이 가득 찬 상태에서는 예약 버튼을 눌러도 영업일을 소비하지 않는다.
...
칩이 올라간 상태라도 예약 구역이 가득 차 있으면 예약 버튼은 비활성화한다.
