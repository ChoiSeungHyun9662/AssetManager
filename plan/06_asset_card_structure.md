...
호일 수치는 배율로 계산하지 않는다.
기획자가 직접 지정한다.
산업 태그는 기술, 소비재, 에너지, 금융, 공업 중 정확히 1개를 가진다.
태그는 카드 자체 효과를 만들지 않고, 미션 조건과 정산 공식, 포트폴리오 방향성에만 사용한다.
...
최종 가치는 기본/호일 가치에 Mr.Market 영구 가치 델타를 반영한 값이다.
0보다 작아질 수 없다.
...
Mr.Market 영구 배당 델타가 있다면 최종 배당금에 반영한다.
최종 배당금은 0보다 작아질 수 없다.
...
```text
- 등급은 기본 주식과 동일
- 기본 수치는 호일 가치와 호일 배당금 사용
- 재료 3장의 Mr.Market 영구 가치/배당 델타를 합산
- 최종 가치와 최종 배당금은 0 미만으로 내려가지 않음
- 무지개 호일 테두리로 표시
```
...
매도 수익은 현금 흐름에 포함한다.
매도 가격은 Mr.Market 영구 가치/배당 델타를 반영하지 않는다.
...
- Mr.Market 영구 가치/배당 델타는 최종 가치, 최종 배당금, 미션 정산에 반영한다.
- 매도 가격은 Mr.Market 영구 델타를 반영하지 않는다.
```
