...
딜 칩은 전문 자원 슬롯에 배치할 수 있는 조커 자원이다.
...
시장 카드 매수는 구매 출처가 `MarketTape`인 매수이다.
...
예약 카드 매수는 구매 출처가 `Reserved`인 매수이다.
...
딜 칩은 전문 자원 슬롯 대체와 현금 할인 효과를 동시에 가진다.
...
구매 출처는 별도로 기록해 출처 기반 규칙에 사용할 수 있다.
...
- 시장 카드 매수는 산 자리만 보충한다.
- 예약 카드 매수는 예약 슬롯만 비운다.
- 예약 카드 매수도 일반 자산 매수로 판정한다.
- 구매 출처는 PurchaseSource로 남긴다.
- 매수 후 자산 인컴은 다음 영업일부터 발생한다.
- 딜 칩 할인은 인플레이션 적용 전에 반영한다.
