...
이 문서는 자원 확보 시스템의 진입 방식, 화면 상태, 자원 선택 규칙, 취소 조건, 완료 조건, 전문 자원 한도 처리 규칙을 정의한다.

자원 확보는 플레이어가 영업일 하나를 사용해 기본 자원을 확보하는 행동이다.

자원 확보는 액션 선택창에서 고르는 방식이 아니다.
플레이어는 시장 영역의 `중앙 은행`을 클릭해 자원 확보 화면으로 진입한다.
...
자원 확보 화면은 별도 팝업이 아니라 시장 영역을 대체하는 상태이다.

---

## 2. 자원 확보의 핵심 성격

자원 확보는 자원을 얻기 위한 기본 행동이다.
...
시장 상태에서 중앙 은행을 클릭하면 자원 확보 화면으로 진입한다.
...
자원 확보에서 첫 자원을 클릭하는 순간, 자원 확보 행동이 확정된다.
...
자원 확보 화면은 첫 자원 획득 전까지만 닫을 수 있다.
...
자원 획득 후에는 자원 확보가 완료될 때까지 진행한다.

---

## 6. 자원 클릭 즉시 획득

자원 확보 화면에서 자원 버튼을 클릭하면 해당 자원을 즉시 획득한다.
...
첫 자원 클릭부터는 해당 영업일의 행동이 자원 확보로 확정된다.
...
자원 확보는 기본적으로 다음 두 조건 중 하나를 만족하면 완료된다.
...
서로 다른 기본 자원 3개를 선택하면 자원 확보가 완료된다.
...
자원 확보 중에는 현재 선택 상태에서 유효한 자원만 활성화한다.
...
## 13. 전문 자원 10/10에서 자원 확보

전문 자원이 이미 10개인 상태에서 자원 확보를 시작하면 현금만 선택할 수 있다.
...
자원 확보는 조합이 정상 완성되지 않았더라도, 현재 상태에서 더 이상 유효한 자원 선택지가 없으면 자동으로 종료한다.
...
자원 확보에서 자원을 하나라도 획득한 뒤에는 다른 기본 행동을 할 수 없다.
...
자원 확보 화면에 진입했지만 아직 자원을 획득하지 않은 상태에서는 닫기를 통해 시장 상태로 복귀할 수 있다.
...
`SelectedResources`는 이번 자원 확보에서 이미 획득한 자원 순서를 기록한다.
...
자원 확보의 선택 상태에 따라 다음 선택이 유효한지 판정한다.
...
자원 확보에서 전문 자원 버튼은 한도를 넘지 않게 비활성화되므로, 일반적으로 초과 폐기가 발생하지 않는다.
...
자원 확보 화면에서는 자원 버튼 상태를 매 선택 후 갱신한다.
