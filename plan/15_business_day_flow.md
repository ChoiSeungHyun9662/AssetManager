...
```text
1. 영업일 시작 상태 진입
2. 보유 자산의 영업일 시작 현금 발생
3. 시장 영역을 Market 상태로 설정
4. 다음 영업일 버튼 상태 갱신
5. 플레이어 입력 대기 상태 진입
```

보유 자산의 영업일 시작 현금은 영업일 시작마다 발생한다.
...
보유 자산의 영업일 시작 현금은 `운용 수익`으로 기록한다.
...
매수한 자산은 매수한 즉시 운용 수익을 발생시키지 않는다.
...
운용 수익 처리가 끝나면 플레이어 입력 대기 상태로 진입한다.
...
또한 자원 확보에서 첫 자원을 획득하면 행동이 확정된다.
...
| 상태 | 의미 |
|---|---|
| Market | 시장 기본 상태. 카드, 중앙 은행, 다음 영업일 조작 가능 |
| CardDetail | 카드 정보와 매수/예약 UI 표시 |
| GainLiquidity | 자원 확보 선택 UI 표시 |
...
자원 확보 상태는 첫 자원 획득 전까지는 검토 상태이다.
...
이 경우 다음 영업일 시작 운용 수익은 발생하지 않는다.
...
```text
- 영업일 시작마다 보유 자산의 영업일 시작 현금이 발생한다.
- 분기 첫 영업일에도 운용 수익은 발생한다.
- 마지막 영업일 종료 후에는 운용 수익 없이 분기 마감으로 간다.
- 카드 보기와 닫기는 영업일을 소비하지 않는다.
- 자원 확보 진입은 영업일을 소비하지 않는다.
- 자원 확보 첫 자원 클릭은 행동 확정이다.
- 매수 확정은 영업일을 소비한다.
- 예약 버튼 클릭은 영업일을 소비한다.
- 다음 영업일 버튼은 Market 상태에서만 활성화된다.
- 다음 영업일은 시장 테이프를 진행시키지 않는다.
- 추가 매수권 상태에서는 자원 확보와 예약을 막는다.
