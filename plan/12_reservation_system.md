...
예약은 시장 카드 1장을 시장에서 빼내 예약 슬롯에 장기 보관하는 행동이다.  
예약은 좋은 카드를 나중에 매수하기 위해 보관하는 수단이며, 동시에 딜 칩을 얻는 주요 수단이다.
...
예약 슬롯이 가득 찬 경우 더 이상 새 카드를 예약할 수 없다.
...
예약 슬롯이 가득 찬 상태에서는 예약 버튼을 눌러도 예약 액션이 실행되지 않는다.  
영업일도 소비하지 않는다.
...
예약 슬롯이 가득 찬 경우에는 예약 버튼이 비활성화되므로, 칩 자동 회수도 발생하지 않는다.
...
시장 테이프 진행과 갱신은 예약 슬롯에 영향을 주지 않는다.
...
예약 슬롯이 모두 차 있으면 시장 카드 상세보기의 예약 버튼은 비활성화된다.
...
- 예약 카드는 운용가치, 인컴, 분기 마감 정산에 포함하지 않는다.
- 예약 카드 매수는 일반 자산 매수로 판정하되 구매 출처는 Reserved로 기록한다.
```
